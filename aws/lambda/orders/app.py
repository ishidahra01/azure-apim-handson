"""
Orders API Service - AWS Lambda
受注情報を提供するLambda Function

このサービスは認証・変換・レート制限などのロジックを含まず、
純粋なビジネスロジックのみを実装しています。
セキュリティやポリシーはAPI Gatewayレイヤーで実装します。
"""
...
def get_order(
    ...
    API Gatewayが認証を行い、検証済みのヘッダー(x-caller-id等)を付与します。
    バックエンドは信頼済みヘッダーを使用してビジネスロジックを実行します。
    """
    # API Gatewayから渡された認証情報をログに記録（デモ用）
    if x_caller_id:
